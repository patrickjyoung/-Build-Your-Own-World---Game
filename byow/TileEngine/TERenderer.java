...
public class TERenderer {
    ...
    private int xOffset;
    private int yOffset;
    private static Map<TETile, String> conversion = Map.of(
                Tileset.FLOOR, "Floor",
                Tileset.WALL, "Wall",
                Tileset.NOTHING, "Nothing",
                Tileset.AVATAR, "Avatar",
                Tileset.LOCKED_DOOR, "Goal",
                Tileset.UNLOCKED_DOOR, "Start Point",
                Tileset.SAND, "COVID AHHHHHHHHH"
        );
     ...
     */
    public void renderFrame(TETile[][] world, int xmouse, int ymouse, int mask, int health) {
        ...
        StdDraw.line(0, world[1].length, world.length,  world[1].length);
        if (xmouse < 0) {
            StdDraw.textLeft(0, world[1].length + 1, "INSTANT REPLAY");
        } else {
            StdDraw.textLeft(0, world[1].length + 1, "Tile: "
                    + conversion.get(world[xmouse][ymouse]));
        }
        if (mask % 2 == 0) {
            StdDraw.textRight(world.length, world[1].length + 1, "Mask: ON");
        } else {
            StdDraw.textRight(world.length, world[1].length + 1, "Mask: OFF");
        }
        StdDraw.text(world.length / 2 , world[1].length + 1, "Public Health: " + health);
